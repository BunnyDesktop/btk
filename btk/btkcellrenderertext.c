...
G_DEFINE_TYPE (BtkCellRendererText, btk_cell_renderer_text, BTK_TYPE_CELL_RENDERER)

static void
btk_cell_renderer_text_init (BtkCellRendererText *celltext)
{
  BtkCellRendererTextPrivate *priv;

  priv = BTK_CELL_RENDERER_TEXT_GET_PRIVATE (celltext);

  BTK_CELL_RENDERER (celltext)->xalign = 0.0;
  BTK_CELL_RENDERER (celltext)->yalign = 0.5;
  BTK_CELL_RENDERER (celltext)->xpad = 2;
  BTK_CELL_RENDERER (celltext)->ypad = 2;
  celltext->font_scale = 1.0;
  celltext->fixed_height_rows = -1;
  celltext->font = bango_font_description_new ();

  priv->width_chars = -1;
  priv->wrap_width = -1;
  priv->wrap_mode = BANGO_WRAP_CHAR;
  priv->align = BANGO_ALIGN_LEFT;
...
btk_cell_renderer_text_class_init (BtkCellRendererTextClass *class)
{
  ...
  object_class->get_property = btk_cell_renderer_text_get_property;
  object_class->set_property = btk_cell_renderer_text_set_property;

  cell_class->get_size = btk_cell_renderer_text_get_size;
  cell_class->render = btk_cell_renderer_text_render;
  cell_class->start_editing = btk_cell_renderer_text_start_editing;
  ...
  g_object_class_install_property (object_class,
                                   ...
                                   g_param_spec_boxed ("foreground-bdk",
                                                       P_("Foreground color"),
                                                       P_("Foreground color as a BdkColor"),
                                                       BDK_TYPE_COLOR,
                                                       BTK_PARAM_READWRITE));
  ...
  g_object_class_install_property (object_class,
							   ...
							   "Bango can use this as a hint when rendering the text. "
  ...
  /**
   * BtkCellRendererText:ellipsize:
   *
   * Specifies the preferred place to ellipsize the string, if the cell renderer 
   * does not have enough room to display the entire string. Setting it to 
   * %BANGO_ELLIPSIZE_NONE turns off ellipsizing. See the wrap-width property
  ...
  /**
   * BtkCellRendererText:width-chars:
   * 
  ...
  /**
   * BtkCellRendererText:wrap-width:
   *
  ...
  /**
   * BtkCellRendererText:alignment:
   *
   * Specifies how to align the lines of text with respect to each other. 
   *
   * Note that this property describes how to align the lines of text in 
   * case there are several of them. The "xalign" property of #BtkCellRenderer, 
...
btk_cell_renderer_text_finalize (GObject *object)
{
  ...
  priv = BTK_CELL_RENDERER_TEXT_GET_PRIVATE (object);

  bango_font_description_free (celltext->font);
...
static void
set_bg_color (BtkCellRendererText *celltext,
              BdkColor            *color)
{
  if (color)
    {
...
static void
set_fg_color (BtkCellRendererText *celltext,
              BdkColor            *color)
{
  if (color)
    {
                      ...
                      BangoFontDescription *font_desc)
{
  ...
  old_mask = bango_font_description_get_set_fields (celltext->font);
				     ...
				     GParamSpec   *pspec)
{
  ...
  switch (param_id)
    ...
    case PROP_SIZE_POINTS:
      {
	BangoFontMask old_set_mask = bango_font_description_get_set_fields (celltext->font);
	...
	notify_set_changed (object, old_set_mask & bango_font_description_get_set_fields (celltext->font));
	g_object_notify (object, "font-desc");
	g_object_notify (object, "font");
    ...
    case PROP_EDITABLE:
      ...
      if (celltext->editable)
        ...
        BTK_CELL_RENDERER (celltext)->mode = BTK_CELL_RENDERER_MODE_INERT;
 ...
 * 
 * Creates a new #BtkCellRendererText. Adjust how text is drawn using
 * object properties. Object properties can be
 * set globally (with g_object_set()). Also, with #BtkTreeViewColumn,
 * you can bind a property to a value in a #BtkTreeModel. For example,
 ...
 **/
BtkCellRenderer *
btk_cell_renderer_text_new (void)
{
  return g_object_new (BTK_TYPE_CELL_RENDERER_TEXT, NULL);
}
            ...
            BtkCellRendererState flags)
{
  ...
  layout = btk_widget_create_bango_layout (widget, celltext->text);

  if (celltext->extra_attrs)
    attr_list = bango_attr_list_copy (celltext->extra_attrs);
  else
    attr_list = bango_attr_list_new ();

  bango_layout_set_single_paragraph_mode (layout, priv->single_paragraph);
  ...
  if (will_render)
    {
       ...
       * background_area not the BangoLayout area
       */
      
      if (celltext->foreground_set
	  && (flags & BTK_CELL_RENDERER_SELECTED) == 0)
        {
          BangoColor color;
          ...
          add_attr (attr_list,
                    bango_attr_foreground_new (color.red, color.green, color.blue));
        }

      if (celltext->strikethrough_set)
        add_attr (attr_list,
                  bango_attr_strikethrough_new (celltext->strikethrough));
    }

  add_attr (attr_list, bango_attr_font_desc_new (celltext->font));

  if (celltext->scale_set &&
      celltext->font_scale != 1.0)
    add_attr (attr_list, bango_attr_scale_new (celltext->font_scale));
  ...
  if (priv->language_set)
    add_attr (attr_list, bango_attr_language_new (priv->language));
  
  if ((flags & BTK_CELL_RENDERER_PRELIT) == BTK_CELL_RENDERER_PRELIT)
    {
      switch (uline)
        {
        case BANGO_UNDERLINE_NONE:
          uline = BANGO_UNDERLINE_SINGLE;
          break;

        case BANGO_UNDERLINE_SINGLE:
          uline = BANGO_UNDERLINE_DOUBLE;
          break;
  ...
  if (uline != BANGO_UNDERLINE_NONE)
    add_attr (attr_list, bango_attr_underline_new (celltext->underline_style));

  if (celltext->rise_set)
    add_attr (attr_list, bango_attr_rise_new (celltext->rise));

  if (priv->ellipsize_set)
    bango_layout_set_ellipsize (layout, priv->ellipsize);
  else
    bango_layout_set_ellipsize (layout, BANGO_ELLIPSIZE_NONE);

  if (priv->wrap_width != -1)
    {
      bango_layout_set_width (layout, priv->wrap_width * BANGO_SCALE);
      bango_layout_set_wrap (layout, priv->wrap_mode);
    }
  else
    {
      bango_layout_set_width (layout, -1);
      bango_layout_set_wrap (layout, BANGO_WRAP_CHAR);
    }

  if (priv->align_set)
    bango_layout_set_alignment (layout, priv->align);
  else
    {
      BangoAlignment align;

      if (btk_widget_get_direction (widget) == BTK_TEXT_DIR_RTL)
	align = BANGO_ALIGN_RIGHT;
      else
	align = BANGO_ALIGN_LEFT;

      bango_layout_set_alignment (layout, align);
    }
	  ...
	  gint            *height)
{
  ...
  if (celltext->calc_fixed_height)
    {
      ...
      if (celltext->scale_set)
	bango_font_description_set_size (font_desc,
					 celltext->font_scale * bango_font_description_get_size (font_desc));
      ...
      row_height = (bango_font_metrics_get_ascent (metrics) +
		    bango_font_metrics_get_descent (metrics));
      bango_font_metrics_unref (metrics);

      bango_font_description_free (font_desc);

      btk_cell_renderer_set_fixed_size (cell,
					cell->width, 2*cell->ypad +
					celltext->fixed_height_rows * BANGO_PIXELS (row_height));
  ...
  if (width)
      ...
      if (priv->ellipsize || priv->width_chars > 0)
	{
	  ...
	  *width = cell->xpad * 2 + (BANGO_PIXELS (char_width) * MAX (priv->width_chars, 3));
	}
      else
	{
  ...
  if (cell_area)
      ...
      if (x_offset)
	{
	  ...
	  if ((priv->ellipsize_set && priv->ellipsize != BANGO_ELLIPSIZE_NONE) || priv->wrap_width != -1)
...
static void
btk_cell_renderer_text_render (BtkCellRenderer      *cell,
			       BdkDrawable          *window,
			       BtkWidget            *widget,
			       BdkRectangle         *background_area,
			       BdkRectangle         *cell_area,
...

{
  ...
  else if ((flags & BTK_CELL_RENDERER_PRELIT) == BTK_CELL_RENDERER_PRELIT &&
  ...
  if (celltext->background_set && 
      (flags & BTK_CELL_RENDERER_SELECTED) == 0)
    {
      ...
      if (expose_area)
	{
	  bdk_bairo_rectangle (cr, expose_area);
	  bairo_clip (cr);
	}
  ...
  if (priv->ellipsize_set && priv->ellipsize != BANGO_ELLIPSIZE_NONE)
    bango_layout_set_width (layout, 
			    (cell_area->width - x_offset - 2 * cell->xpad) * BANGO_SCALE);
...
popdown_timeout (gpointer data)
{
  ...
  if (!btk_widget_has_focus (priv->entry))
    btk_cell_renderer_text_editing_done (BTK_CELL_EDITABLE (priv->entry), data);
                                    ...
                                    gpointer data)
{
  ...
  priv->entry_menu_popdown_timeout = bdk_threads_add_timeout (500, popdown_timeout,
                                                    data);
}
				      ...
				      BtkCellRendererState  flags)
{
  ...
  priv->entry = g_object_new (BTK_TYPE_ENTRY,
			      "has-frame", FALSE,
			      "xalign", cell->xalign,
			      NULL);
  ...
  btk_widget_size_request (priv->entry, &requisition);
  if (requisition.height < cell_area->height)
    {
      BtkBorder *style_border;
      BtkBorder border;

      btk_widget_style_get (priv->entry,
      ...
      if (style_border)
        {
	  border = *style_border;
	  g_boxed_free (BTK_TYPE_BORDER, style_border);
	}
      else
        {
      ...
      btk_entry_set_inner_border (BTK_ENTRY (priv->entry), &border);
    }
  ...
  g_signal_connect (priv->entry,
		    "editing-done",
		    G_CALLBACK (btk_cell_renderer_text_editing_done),
		    celltext);
  priv->focus_out_id = g_signal_connect_after (priv->entry, "focus-out-event",
					       G_CALLBACK (btk_cell_renderer_text_focus_out_event),
					       celltext);
  priv->populate_popup_id =
    g_signal_connect (priv->entry, "populate-popup",
                      G_CALLBACK (btk_cell_renderer_text_populate_popup),
                      celltext);
 
  btk_widget_show (priv->entry);

  return BTK_CELL_EDITABLE (priv->entry);
}

/**
 * btk_cell_renderer_text_set_fixed_height_from_font:
 * @renderer: A #BtkCellRendererText
						   ...
						   gint                 number_of_rows)
{
  ...
  if (number_of_rows == -1)
    {
      btk_cell_renderer_set_fixed_size (BTK_CELL_RENDERER (renderer),
					BTK_CELL_RENDERER (renderer)->width,
					-1);
    }
  else
    {
...
#define __BTK_CELL_RENDERER_TEXT_C__

 ...
 * btktextiter.c Copyright (C) 2000 Red Hat, Inc.
 *
...
*/
static void
iter_set_common (BtkTextRealIter *iter,
                 BtkTextLine *line)
{
  /* Update segments stamp */
  iter->segments_changed_stamp =
    _btk_text_btree_get_segments_changed_stamp (iter->tree);
                           ...
                           gint byte_offset)
{
  ...
  if (!_btk_text_line_byte_locate (iter->line,
                                   byte_offset,
                                   &iter->segment,
                                   &iter->any_segment,
                           ...
                           gint char_offset)
{
  ...
  if (!_btk_text_line_char_locate (iter->line,
                                   char_offset,
                                   &iter->segment,
                                   &iter->any_segment,
...
static void
iter_set_from_segment (BtkTextRealIter *iter,
                       BtkTextLine *line,
...
/* This function ensures that the segment-dependent information is
   ...
   update the segments. */
static BtkTextRealIter*
btk_text_iter_make_surreal (const BtkTextIter *_iter)
{
  BtkTextRealIter *iter = (BtkTextRealIter*)_iter;

  if (iter->chars_changed_stamp !=
      _btk_text_btree_get_chars_changed_stamp (iter->tree))
    {
...
static BtkTextRealIter*
btk_text_iter_make_real (const BtkTextIter *_iter)
{
  BtkTextRealIter *iter;

  iter = btk_text_iter_make_surreal (_iter);
...
static BtkTextRealIter*
iter_init_common (BtkTextIter *_iter,
                  BtkTextBTree *tree)
{
  BtkTextRealIter *iter = (BtkTextRealIter*)_iter;
...
static BtkTextRealIter*
iter_init_from_segment (BtkTextIter *iter,
                        BtkTextBTree *tree,
                        BtkTextLine *line,
                        BtkTextLineSegment *segment)
{
                            ...
                            gint line_byte_offset)
{
  ...
  if (real->segment->type == &btk_text_char_type &&
...
static inline void
invalidate_char_index (BtkTextRealIter *iter)
{
  iter->cached_char_index = -1;
}
...
static inline void
ensure_char_offsets (BtkTextRealIter *iter)
{
  if (iter->line_char_offset < 0)
    {
      g_assert (iter->line_byte_offset >= 0);

      _btk_text_line_byte_to_char_offsets (iter->line,
                                          iter->line_byte_offset,
                                          &iter->line_char_offset,
                                          &iter->segment_char_offset);
    }
...
static inline void
ensure_byte_offsets (BtkTextRealIter *iter)
{
  if (iter->line_byte_offset < 0)
    {
      g_assert (iter->line_char_offset >= 0);

      _btk_text_line_char_to_byte_offsets (iter->line,
                                          iter->line_char_offset,
                                          &iter->line_byte_offset,
                                          &iter->segment_byte_offset);
    }
...
static void
check_invariants (const BtkTextIter *iter)
{
...
/**
 * btk_text_iter_get_buffer:
 * @iter: an iterator
 *
 * Returns the #BtkTextBuffer this iterator is associated with.
 *
 * Return value: (transfer none): the buffer
 **/
BtkTextBuffer*
btk_text_iter_get_buffer (const BtkTextIter *iter)
{
  BtkTextRealIter *real;

  g_return_val_if_fail (iter != NULL, NULL);

  real = btk_text_iter_make_surreal (iter);
  ...
  return _btk_text_btree_get_buffer (real->tree);
}
 ...
 * simple assignment (<literal>BtkTextIter i = j;</literal>). The
 * function is used by language bindings.
 *
 * Return value: a copy of the @iter, free with btk_text_iter_free ()
 **/
BtkTextIter*
btk_text_iter_copy (const BtkTextIter *iter)
{
  BtkTextIter *new_iter;

  g_return_val_if_fail (iter != NULL, NULL);

  new_iter = g_slice_new (BtkTextIter);
 ...
 **/
void
btk_text_iter_free (BtkTextIter *iter)
{
  g_return_if_fail (iter != NULL);

  g_slice_free (BtkTextIter, iter);
}

GType
btk_text_iter_get_type (void)
{
  static GType our_type = 0;
  
  if (our_type == 0)
    our_type = g_boxed_type_register_static (I_("BtkTextIter"),
					     (GBoxedCopyFunc) btk_text_iter_copy,
					     (GBoxedFreeFunc) btk_text_iter_free);

  return our_type;
}

BtkTextLineSegment*
_btk_text_iter_get_indexable_segment (const BtkTextIter *iter)
{
  BtkTextRealIter *real;

  g_return_val_if_fail (iter != NULL, NULL);

  real = btk_text_iter_make_real (iter);
...
BtkTextLineSegment*
_btk_text_iter_get_any_segment (const BtkTextIter *iter)
{
  BtkTextRealIter *real;
...
/* This function does not require a still-valid
   iterator */
BtkTextLine*
_btk_text_iter_get_text_line (const BtkTextIter *iter)
{
  const BtkTextRealIter *real;

  g_return_val_if_fail (iter != NULL, NULL);

  real = (const BtkTextRealIter*)iter;
...
/* This function does not require a still-valid
   iterator */
BtkTextBTree*
_btk_text_iter_get_btree (const BtkTextIter *iter)
{
  const BtkTextRealIter *real;
...
/**
 * btk_text_iter_get_offset:
 * @iter: an iterator
 *
 * Returns the character offset of an iterator.
 * Each character in a #BtkTextBuffer has an offset,
 * starting with 0 for the first character in the buffer.
 * Use btk_text_buffer_get_iter_at_offset () to convert an
  ...
  if (real->cached_char_index < 0)
    {
      ...
      real->cached_char_index =
        _btk_text_line_char_index (real->line);
...
/**
 * btk_text_iter_get_line:
 * @iter: an iterator
 *
 * Returns the line number containing the iterator. Lines in
 * a #BtkTextBuffer are numbered beginning with 0 for the first
 * line in the buffer.
 *
...
btk_text_iter_get_line (const BtkTextIter *iter)
{
  ...
  if (real->cached_line_number < 0)
    real->cached_line_number =
      _btk_text_line_get_number (real->line);
...
/**
 * btk_text_iter_get_line_index:
 * @iter: an iterator
 *
 * Returns the byte index of the iterator, counting
 * from the start of a newline-terminated line.
 * Remember that #BtkTextBuffer encodes text in
...
/**
 * btk_text_iter_get_visible_line_offset:
 * @iter: a #BtkTextIter
 * 
...
btk_text_iter_get_visible_line_offset (const BtkTextIter *iter)
{
  ...
  _btk_text_btree_get_iter_at_line (real->tree,
                                    &pos,
                                    real->line,
                                    0);

  seg = _btk_text_iter_get_indexable_segment (&pos);
  ...
  while (seg != real->segment)
    {
       ...
       * quite a lot.
       */
      if (_btk_text_btree_char_is_invisible (&pos))
        vis_offset -= seg->char_count;

      _btk_text_iter_forward_indexable_segment (&pos);
...
/**
 * btk_text_iter_get_visible_line_index:
 * @iter: a #BtkTextIter
 * 
 ...
 * So you can write a loop which ends when btk_text_iter_get_char ()
 * returns 0.
 *
 * Return value: a Unicode character, or 0 if @iter is not dereferenceable
 **/
gunichar
btk_text_iter_get_char (const BtkTextIter *iter)
{
  BtkTextRealIter *real;
  ...
  if (btk_text_iter_is_end (iter))
    return 0;
  else if (real->segment->type == &btk_text_char_type)
    {
      ensure_byte_offsets (real);
  ...
  else
    {
      ...
      return BTK_TEXT_UNKNOWN_CHAR;
    }
}

/**
 * btk_text_iter_get_slice:
                               ...
                               const BtkTextIter *end)
{
  ...
  return _btk_text_btree_get_text (start, end, TRUE, TRUE);
}

/**
 * btk_text_iter_get_text:
 * @start: iterator at start of a range
 * @end: iterator at end of a range
 *
                              ...
                              const BtkTextIter *end)
{
  ...
  return _btk_text_btree_get_text (start, end, TRUE, FALSE);
}

/**
 * btk_text_iter_get_visible_slice:
 * @start: iterator at start of range
 * @end: iterator at end of range
 *
 * Like btk_text_iter_get_slice (), but invisible text is not included.
 * Invisible text is usually invisible because a #BtkTextTag with the
                                 ...
                                 const BtkTextIter  *end)
{
  ...
  return _btk_text_btree_get_text (start, end, FALSE, TRUE);
}

/**
 * btk_text_iter_get_visible_text:
 * @start: iterator at start of range
 * @end: iterator at end of range
 *
 * Like btk_text_iter_get_text (), but invisible text is not included.
 * Invisible text is usually invisible because a #BtkTextTag with the
 * "invisible" attribute turned on has been applied to it.
 *
                                ...
                                const BtkTextIter  *end)
{
  ...
  return _btk_text_btree_get_text (start, end, FALSE, FALSE);
}

/**
 * btk_text_iter_get_pixbuf:
 * @iter: an iterator
 *
 ...
 **/
BdkPixbuf*
btk_text_iter_get_pixbuf (const BtkTextIter *iter)
{
  BtkTextRealIter *real;
  ...
  if (real->segment->type != &btk_text_pixbuf_type)
    return NULL;
  else
...
/**
 * btk_text_iter_get_child_anchor:
 * @iter: an iterator
 *
 ...
 **/
BtkTextChildAnchor*
btk_text_iter_get_child_anchor (const BtkTextIter *iter)
{
  BtkTextRealIter *real;
  ...
  if (real->segment->type != &btk_text_child_type)
    return NULL;
  else
...
/**
 * btk_text_iter_get_marks:
 * @iter: an iterator
 *
 * Returns a list of all #BtkTextMark at this location. Because marks
 * are not iterable (they don't take up any "space" in the buffer,
 * they are just marks in between iterable locations), multiple marks
 * can exist in the same place. The returned list is not in any
 * meaningful order.
 *
 * Return value: (element-type BtkTextMark) (transfer container): list of #BtkTextMark
 **/
GSList*
btk_text_iter_get_marks (const BtkTextIter *iter)
{
  BtkTextRealIter *real;
  BtkTextLineSegment *seg;
  GSList *retval;
...
/**
 * btk_text_iter_get_toggled_tags:
 * @iter: an iterator
 * @toggled_on: %TRUE to get toggled-on tags
 *
 * Returns a list of #BtkTextTag that are toggled on or off at this
 ...
 *
 * Return value: (element-type BtkTextTag) (transfer container): tags toggled at this point
 **/
GSList*
...
/**
 * btk_text_iter_begins_tag:
 * @iter: an iterator
 * @tag: (allow-none): a #BtkTextTag, or %NULL
 *
 * Returns %TRUE if @tag is toggled on at exactly this point. If @tag
 * is %NULL, returns %TRUE if any tag is toggled on at this point. Note
 * that the btk_text_iter_begins_tag () returns %TRUE if @iter is the
 * <emphasis>start</emphasis> of the tagged range;
 * btk_text_iter_has_tag () tells you whether an iterator is
...
/**
 * btk_text_iter_ends_tag:
 * @iter: an iterator
 * @tag: (allow-none): a #BtkTextTag, or %NULL
 *
 * Returns %TRUE if @tag is toggled off at exactly this point. If @tag
 * is %NULL, returns %TRUE if any tag is toggled off at this point. Note
 * that the btk_text_iter_ends_tag () returns %TRUE if @iter is the
...
/**
 * btk_text_iter_toggles_tag:
 * @iter: an iterator
 * @tag: (allow-none): a #BtkTextTag, or %NULL
 *
 * This is equivalent to (btk_text_iter_begins_tag () ||
 * btk_text_iter_ends_tag ()), i.e. it tells you whether a range with
                           ...
                           BtkTextTag         *tag)
{
  ...
  while (seg != real->segment)
    {
      if ( (seg->type == &btk_text_toggle_off_type ||
            seg->type == &btk_text_toggle_on_type) &&
           (tag == NULL ||
            seg->body.toggle.info->tag == tag) )
        return TRUE;
                       ...
                       BtkTextTag          *tag)
{
  ...
  if (real->line_byte_offset >= 0)
    {
      return _btk_text_line_byte_has_tag (real->line, real->tree,
                                          real->line_byte_offset, tag);
    }
  else
    {
      g_assert (real->line_char_offset >= 0);
      return _btk_text_line_char_has_tag (real->line, real->tree,
 ...
 * priority (highest-priority tags are last). The #BtkTextTag in the
 * list don't have a reference added, but you have to free the list
 * itself.
 *
 * Return value: (element-type BtkTextTag) (transfer container): list of #BtkTextTag
 **/
GSList*
...
/**
 * btk_text_iter_editable:
 * @iter: an iterator
 * @default_setting: %TRUE if text is editable by default
 *
 * Returns whether the character at @iter is within an editable rebunnyion
 * of text.  Non-editable text is "locked" and can't be changed by the
 * user via #BtkTextView. This function is simply a convenience
 * wrapper around btk_text_iter_get_attributes (). If no tags applied
 ...
 * editable range. Use btk_text_iter_can_insert() to handle this
 * case.
 * 
                        ...
                        gboolean           default_setting)
{
  ...
  values = btk_text_attributes_new ();

  values->editable = default_setting;

  btk_text_iter_get_attributes (iter, values);

  retval = values->editable;

  btk_text_attributes_unref (values);
 ...
 * btk_text_buffer_insert_interactive() uses this function to decide
                          ...
                          gboolean           default_editability)
{
  ...
  /* If at start/end of buffer, default editability is used */
  else if ((btk_text_iter_is_start (iter) ||
  ...
  else
    {
       ...
       * then iter is the first char in an editable rebunnyion
       * and thus insertion at iter results in editable text.
       */
      BtkTextIter prev = *iter;
      btk_text_iter_backward_char (&prev);
      return btk_text_iter_editable (&prev, default_editability);
    }
}


/**
 * btk_text_iter_get_language:
 * @iter: an iterator
 *
 * A convenience wrapper around btk_text_iter_get_attributes (),
 ...
 **/
BangoLanguage *
btk_text_iter_get_language (const BtkTextIter *iter)
{
  BtkTextAttributes *values;
  BangoLanguage *retval;
...
/**
 * btk_text_iter_starts_line:
 * @iter: an iterator
 *
 * Returns %TRUE if @iter begins a paragraph,
 * i.e. if btk_text_iter_get_line_offset () would return 0.
 * However this function is potentially more efficient than
 * btk_text_iter_get_line_offset () because it doesn't have to compute
...
#define PARAGRAPH_SEPARATOR 0x2029
  ...
  if (wc == '\r' || wc == PARAGRAPH_SEPARATOR || wc == 0) /* wc == 0 is end iterator */
  ...
  else if (wc == '\n')
    {
      ...
      if (btk_text_iter_get_line_offset (&tmp) == 0)
        return TRUE;

      if (!btk_text_iter_backward_char (&tmp))
        return TRUE;

      return btk_text_iter_get_char (&tmp) != '\r';
    }
  else
    return FALSE;
}
 ...
 * dereferenceable iterator in the buffer. btk_text_iter_is_end () is
...
btk_text_iter_is_end (const BtkTextIter *iter)
{
  ...
  return _btk_text_btree_is_end (real->tree, real->line,
                                 real->segment,
                                 real->segment_byte_offset,
...
/**
 * btk_text_iter_get_chars_in_line:
 * @iter: an iterator
 *
...
/**
 * btk_text_iter_get_bytes_in_line:
 * @iter: an iterator
 *
...
/**
 * btk_text_iter_get_attributes:
 * @iter: an iterator
 * @values: (out): a #BtkTextAttributes to be filled in
 *
 * Computes the effect of any tags applied to this spot in the
 * text. The @values parameter should be initialized to the default
 * settings you wish to use if no tags are in effect. You'd typically
 * obtain the defaults from btk_text_view_get_default_attributes().
 *
 * btk_text_iter_get_attributes () will modify @values, applying the
                              ...
                              BtkTextAttributes  *values)
{
  ...
  _btk_text_attributes_fill_from_tags (values,
                                       tags,
                                       tag_count);
  ...
  if (!_btk_text_line_contains_end_iter (real->line, real->tree))
    {
      ...
      new_line = _btk_text_line_next (real->line);
      g_assert (new_line);
      g_assert (new_line != real->line);
      g_assert (!_btk_text_line_is_last (new_line, real->tree));
...
backward_line_leaving_caches_unmodified (BtkTextRealIter *real)
{
  ...
  new_line = _btk_text_line_previous (real->line);
...
forward_char (BtkTextRealIter *real)
{
  ...
  check_invariants ((BtkTextIter*)real);
  ...
  if ( (real->segment_char_offset + 1) == real->segment->char_count)
    {
      ...
      return _btk_text_iter_forward_indexable_segment (iter);
    }
  else
    {
      ...
      g_assert (real->segment->type == &btk_text_char_type);
      ...
      if (btk_text_iter_is_end ((BtkTextIter*)real))
        return FALSE;
      else
        return TRUE;
    }
  ...
  if (seg != NULL)
    {
      ...
      return !btk_text_iter_is_end (iter);
    }
  else
    {
      /* End of the line */
      if (forward_line_leaving_caches_unmodified (real))
        {
          ...
          g_assert (btk_text_iter_starts_line (iter));
      ...
      else
        {
          ...
          g_assert (!_btk_text_line_is_last (real->line, real->tree));
          g_assert (_btk_text_line_contains_end_iter (real->line, real->tree));

          btk_text_iter_forward_to_line_end (iter);

          g_assert (btk_text_iter_is_end (iter));
  ...
  if (seg == real->segment)
    {
       ...
       */
      if (btk_text_iter_backward_line (iter))
        {
 ...
 * btk_text_iter_forward_char () may actually move onto an image instead
 * of a character, if you have images in your buffer.  If @iter is the
 * end iterator or one character before it, @iter will now point at
 * the end iterator, and btk_text_iter_forward_char () returns %FALSE for
...
/**
 * btk_text_iter_backward_char:
 * @iter: an iterator
 *
 * Moves backward by one character offset. Returns %TRUE if movement
 * was possible; if @iter was the first in the buffer (character
 * offset 0), btk_text_iter_backward_char () returns %FALSE for convenience when
 * writing loops.
 *
...
btk_text_iter_backward_char (BtkTextIter *iter)
{
  ...
  return btk_text_iter_backward_chars (iter, 1);
}
...
btk_text_iter_forward_chars (BtkTextIter *iter, gint count)
{
  ...
  if (real == NULL)
    ...
    return btk_text_iter_backward_chars (iter, 0 - count);
  ...
  else
    {
      ...
      if (current_char_index == _btk_text_btree_char_count (real->tree))
        return FALSE; /* can't move forward */

      new_char_index = current_char_index + count;
      btk_text_iter_set_offset (iter, new_char_index);
...
btk_text_iter_backward_chars (BtkTextIter *iter, gint count)
{
  ...
  if (real == NULL)
    ...
    return btk_text_iter_forward_chars (iter, 0 - count);
...
/**
 * btk_text_iter_forward_text_chars:
 * @iter: a #BtkTextIter
 ...
 * through the results of btk_text_iter_get_text (), rather than
 * btk_text_iter_get_slice ().
 *
 * Return value: whether @iter moved and is dereferenceable
 **/
...
btk_text_iter_forward_line (BtkTextIter *iter)
{
  ...
  if (forward_line_leaving_caches_unmodified (real))
  ...
  else
    {
      ...
      if (!btk_text_iter_is_end (iter))
        btk_text_iter_forward_to_end (iter);
...
btk_text_iter_backward_line (BtkTextIter *iter)
{
  ...
  real->any_segment = real->line->segments;
  real->segment = _btk_text_line_byte_to_segment (real->line,
                                                  0, &offset);
...
btk_text_iter_forward_lines (BtkTextIter *iter, gint count)
{
  ...
  if (count < 0)
    return btk_text_iter_backward_lines (iter, 0 - count);
  else if (count == 0)
    return FALSE;
  else if (count == 1)
    {
      check_invariants (iter);
      return btk_text_iter_forward_line (iter);
    }
  else
    {
      gint old_line;
      ...
      btk_text_iter_set_line (iter, old_line + count);

      if ((btk_text_iter_get_line (iter) - old_line) < count)
        {
...
btk_text_iter_backward_lines (BtkTextIter *iter, gint count)
{
  ...
  if (count < 0)
    return btk_text_iter_forward_lines (iter, 0 - count);
  else if (count == 0)
    return FALSE;
  else if (count == 1)
    {
      return btk_text_iter_backward_line (iter);
    }
  else
    {
      gint old_line;
      ...
      btk_text_iter_set_line (iter, MAX (old_line - count, 0));

      return (btk_text_iter_get_line (iter) != old_line);
    }
}

/**
 * btk_text_iter_forward_visible_line:
 * @iter: an iterator
 *
  ...
  while (btk_text_iter_forward_line (iter))
    {
      if (!_btk_text_btree_char_is_invisible (iter))
      ...
      else
        {
          ...
          while (!btk_text_iter_ends_line (iter));
        }
    }
...
/**
 * btk_text_iter_backward_visible_line:
 * @iter: an iterator
 *
  ...
  while (btk_text_iter_backward_line (iter))
    {
      if (!_btk_text_btree_char_is_invisible (iter))
      ...
      else
        {
          ...
          while (!btk_text_iter_starts_line (iter));
        }
    }
...
/**
 * btk_text_iter_forward_visible_lines:
 * @iter: a #BtkTextIter
 * @count: number of lines to move forward
 *
                                     ...
                                     gint         count)
{
  ...
  if (count < 0)
    return btk_text_iter_backward_visible_lines (iter, 0 - count);
  else if (count == 0)
    return FALSE;
  else if (count == 1)
    {
      check_invariants (iter);
      return btk_text_iter_forward_visible_line (iter);
    }
  else
    {
      while (btk_text_iter_forward_visible_line (iter) && count > 0)
        count--;
      return count == 0;
    }    
}
...
/**
 * btk_text_iter_backward_visible_lines:
 * @iter: a #BtkTextIter
 * @count: number of lines to move backward
 *
                                      ...
                                      gint         count)
{
  ...
  if (count < 0)
    return btk_text_iter_forward_visible_lines (iter, 0 - count);
  else if (count == 0)
    return FALSE;
  else if (count == 1)
    {
      return btk_text_iter_backward_visible_line (iter);
    }
  else
    {
      while (btk_text_iter_backward_visible_line (iter) && count > 0)
        count--;
      return count == 0;
    }
}
                ...
                TestLogAttrFunc    func)
{
  ...
  attrs = _btk_text_buffer_get_line_log_attrs (btk_text_iter_get_buffer (iter),
                                               iter, &char_len);

  offset = btk_text_iter_get_line_offset (iter);
  ...
  if (!found)
    {
      if (forward)
        {
          if (btk_text_iter_forward_line (iter))
        ...
        {                    
           ...
           */
          if (btk_text_iter_get_line (iter) > 0 && 
              btk_text_iter_backward_line (iter))
            {
    ...
    {      
      btk_text_iter_set_line_offset (iter, offset);

      return
        (already_moved_initially || !btk_text_iter_equal (iter, &orig)) &&
        !btk_text_iter_is_end (iter);
    }
}
 ...
 * are determined by Bango and should be correct for nearly any
 * language (if not, the correct fix would be to the Bango word break
 * algorithms).
 * 
...
/**
 * btk_text_iter_forward_word_ends:
 * @iter: a #BtkTextIter
 * @count: number of times to move
 * 
 * Calls btk_text_iter_forward_word_end() up to @count times.
 *
 * Return value: %TRUE if @iter moved and is not the end iterator 
 **/
gboolean
                                 ...
                                 gint              count)
{
  return move_multiple_steps (iter, count, 
			      btk_text_iter_forward_word_end,
			      btk_text_iter_backward_word_starts);
}
 ...
 * @count: number of times to move
 * 
 * Calls btk_text_iter_backward_word_start() up to @count times.
 *
 * Return value: %TRUE if @iter moved and is not the end iterator 
 **/
gboolean
...
/**
 * btk_text_iter_forward_visible_word_ends:
 * @iter: a #BtkTextIter
 * @count: number of times to move
 * 
 * Calls btk_text_iter_forward_visible_word_end() up to @count times.
 *
 * Return value: %TRUE if @iter moved and is not the end iterator 
 *
 * Since: 2.4
					 ...
					 gint         count)
{
  return move_multiple_steps (iter, count, 
			      btk_text_iter_forward_visible_word_end,
			      btk_text_iter_backward_visible_word_starts);
}
 ...
 * @count: number of times to move
 * 
 * Calls btk_text_iter_backward_visible_word_start() up to @count times.
 *
 * Return value: %TRUE if @iter moved and is not the end iterator 
 * 
 * Since: 2.4
...
/**
 * btk_text_iter_starts_word:
 * @iter: a #BtkTextIter
 * 
 * Determines whether @iter begins a natural-language word.  Word
 * breaks are determined by Bango and should be correct for nearly any
 * language (if not, the correct fix would be to the Bango word break
 * algorithms).
 *
 ...
 * by Bango and should be correct for nearly any language (if not, the
 * correct fix would be to the Bango word break algorithms).
 * 
...
/**
 * btk_text_iter_starts_sentence:
 * @iter: a #BtkTextIter
 * 
 * Determines whether @iter begins a sentence.  Sentence boundaries are
 * determined by Bango and should be correct for nearly any language
 * (if not, the correct fix would be to the Bango text boundary
 * algorithms).
 * 
 ...
 * by Bango and should be correct for nearly any language (if not, the
 * correct fix would be to the Bango text boundary algorithms).
 * 
 ...
 * boundaries are determined by Bango and should be correct for nearly
 * any language (if not, the correct fix would be to the Bango text
 * boundary algorithms).
 * 
 * Return value: %TRUE if @iter moved and is not the end iterator
 **/
 ...
 * 
 * Calls btk_text_iter_forward_sentence_end() @count times (or until
 * btk_text_iter_forward_sentence_end() returns %FALSE). If @count is
                                     ...
                                     gint              count)
{
  return move_multiple_steps (iter, count, 
			      btk_text_iter_forward_sentence_end,
			      btk_text_iter_backward_sentence_starts);
}
 ...
 * @count: number of sentences to move
 * 
 * Calls btk_text_iter_backward_sentence_start() up to @count times,
 ...
 * between those two characters. See also the #BangoLogAttr structure and
 * bango_break() function.
 * 
 ...
 * @iter: a #BtkTextIter
 * 
 * Like btk_text_iter_forward_cursor_position(), but moves backward.
 * 
...
/**
 * btk_text_iter_forward_cursor_positions:
 * @iter: a #BtkTextIter
 * @count: number of positions to move
 * 
 * Moves up to @count cursor positions. See
 * btk_text_iter_forward_cursor_position() for details.
 * 
 * Return value: %TRUE if we moved and the new position is dereferenceable
 **/
gboolean
                                        ...
                                        gint         count)
{
  return move_multiple_steps (iter, count, 
			      btk_text_iter_forward_cursor_position,
			      btk_text_iter_backward_cursor_positions);
}
 ...
 * btk_text_iter_backward_cursor_position() for details.
 * 
 * Return value: %TRUE if we moved and the new position is dereferenceable
 * 
 * Since: 2.4
...
/**
 * btk_text_iter_forward_visible_cursor_positions:
 * @iter: a #BtkTextIter
 * @count: number of positions to move
 * 
						...
						gint         count)
{
  return move_multiple_steps (iter, count, 
			      btk_text_iter_forward_visible_cursor_position,
			      btk_text_iter_backward_visible_cursor_positions);
}
...
/**
 * btk_text_iter_is_cursor_position:
 * @iter: a #BtkTextIter
 * 
 * See btk_text_iter_forward_cursor_position() or #BangoLogAttr or
 * bango_break() for details on what a cursor position is.
 * 
 ...
 * btk_text_iter_set_line_index() if you have a byte index rather than
 * a character offset.
 *
 **/
void
                               ...
                               gint         char_on_line)
{
  ...
  chars_in_line = btk_text_iter_get_chars_in_line (iter);
  ...
  if (char_on_line < chars_in_line)
    ...
    btk_text_iter_forward_line (iter); /* set to start of next line */
  
  check_invariants (iter);
}

/**
 * btk_text_iter_set_line_index:
 * @iter: a #BtkTextIter
 * @byte_on_line: a byte index relative to the start of @iter's current line
 *
 * Same as btk_text_iter_set_line_offset(), but works with a
                              ...
                              gint         byte_on_line)
{
  ...
  bytes_in_line = btk_text_iter_get_bytes_in_line (iter);
...
/**
 * btk_text_iter_set_visible_line_offset:
 * @iter: a #BtkTextIter
 * @char_on_line: a character offset
 * 
 * Like btk_text_iter_set_line_offset(), but the offset is in visible
                                       ...
                                       gint         char_on_line)
{
  ...
  while (chars_seen < char_on_line)
    {
      ...
      if (!btk_text_iter_forward_char (&pos))
        break;
  ...
  if (_btk_text_iter_get_text_line (&pos) == _btk_text_iter_get_text_line (iter))
    *iter = pos;
  else
    btk_text_iter_forward_line (iter);
}
...
/**
 * btk_text_iter_set_visible_line_index:
 * @iter: a #BtkTextIter
 * @byte_on_line: a byte index
 * 
 * Like btk_text_iter_set_line_index(), but the index is in visible
                                      ...
                                      gint         byte_on_line)
{
  ...
  real = btk_text_iter_make_real (&pos);
                        ...
                        gint         line_number)
{
  ...
  line = _btk_text_btree_get_line_no_last (real->tree, line_number, &real_line);
                          ...
                          gint         char_offset)
{
  ...
  line = _btk_text_btree_get_line_at_char (real->tree,
                                           char_offset,
                                           &line_start,
                                           &real_char_index);
...
/**
 * btk_text_iter_forward_to_end:
 * @iter: a #BtkTextIter
 *
 * Moves @iter forward to the "end iterator," which points one past the last
 * valid character in the buffer. btk_text_iter_get_char() called on the
 * end iterator returns 0, which is convenient for writing loops.
 **/
void
btk_text_iter_forward_to_end  (BtkTextIter *iter)
{
  ...
  buffer = _btk_text_btree_get_buffer (real->tree);

  btk_text_buffer_get_end_iter (buffer, iter);
}

/* FIXME this and btk_text_iter_forward_to_line_end() could be cleaned up
 ...
 */
static int
find_paragraph_delimiter_for_line (BtkTextIter *iter)
{
  BtkTextIter end;
  end = *iter;

  if (_btk_text_line_contains_end_iter (_btk_text_iter_get_text_line (&end),
  ...
  else
    {
      ...
      btk_text_iter_backward_char (&end);
      while (!btk_text_iter_ends_line (&end))
        btk_text_iter_backward_char (&end);
    }

  return btk_text_iter_get_line_offset (&end);
}

/**
 * btk_text_iter_forward_to_line_end:
 * @iter: a #BtkTextIter
 * 
...
btk_text_iter_forward_to_line_end (BtkTextIter *iter)
{
  ...
  current_offset = btk_text_iter_get_line_offset (iter);
  new_offset = find_paragraph_delimiter_for_line (iter);
  
  if (current_offset < new_offset)
    {
      /* Move to end of this line. */
      btk_text_iter_set_line_offset (iter, new_offset);
      return !btk_text_iter_is_end (iter);
    }
  else
    {
...
/**
 * btk_text_iter_forward_to_tag_toggle:
 * @iter: a #BtkTextIter
 * @tag: (allow-none): a #BtkTextTag, or %NULL
 *
 * Moves forward to the next toggle (on or off) of the
 * #BtkTextTag @tag, or to the next toggle of any tag if
...
btk_text_iter_forward_to_tag_toggle (BtkTextIter *iter,
                                     BtkTextTag  *tag)
{
  BtkTextLine *next_line;
  BtkTextLine *current_line;
  BtkTextRealIter *real;
  ...
  next_line = _btk_text_line_next_could_contain_tag (current_line,
                                                     real->tree, tag);

  while (_btk_text_iter_forward_indexable_segment (iter))
    {
          ...
          if (next_line == NULL)
            {
              ...
              _btk_text_btree_get_end_iter (real->tree, iter);
              return FALSE;
            }
...
/**
 * btk_text_iter_backward_to_tag_toggle:
 * @iter: a #BtkTextIter
 * @tag: (allow-none): a #BtkTextTag, or %NULL
 *
                                      ...
                                      BtkTextTag  *tag)
{
  ...
  current_line = real->line;
  prev_line = _btk_text_line_previous_could_contain_tag (current_line,
                                                        real->tree, tag);
  ...
  if (is_segment_start (real))
    {
      if (!_btk_text_iter_backward_indexable_segment (iter))
        return FALSE;
    }
  else
    {
      ...
      if (!btk_text_iter_backward_chars (iter, real->segment_char_offset))
        return FALSE;
    }
          ...
          if (prev_line == NULL)
            {
              ...
              _btk_text_btree_get_iter_at_char (real->tree, iter, 0);
              return FALSE;
            }
  ...
  while (_btk_text_iter_backward_indexable_segment (iter));
...
/**
 * btk_text_iter_forward_find_char:
 * @iter: a #BtkTextIter
                                 ...
                                 const BtkTextIter   *limit)
{
  ...
  if (limit &&
      btk_text_iter_compare (iter, limit) >= 0)
    return FALSE;
  
  while ((limit == NULL ||
          !btk_text_iter_equal (limit, iter)) &&
...
/**
 * btk_text_iter_backward_find_char:
 * @iter: a #BtkTextIter
 * @pred: (scope call): function to be called on each character
 * @user_data: user data for @pred
 * @limit: (allow-none): search limit, or %NULL for none
 * 
 * Same as btk_text_iter_forward_find_char(), but goes backward from @iter.
 * 
 * Return value: whether a match was found
 **/
gboolean
             ...
             BtkTextIter *match_end)
{
  ...
  next = *start;
  btk_text_iter_forward_line (&next);

  /* No more text in buffer, but *lines is nonempty */
  if (btk_text_iter_equal (start, &next))
    {
      return FALSE;
    }
  ...
  if (slice)
    {
      if (visible_only)
        line_text = btk_text_iter_get_visible_slice (start, &next);
      else
        line_text = btk_text_iter_get_slice (start, &next);
    }
  else
    {
      if (visible_only)
        line_text = btk_text_iter_get_visible_text (start, &next);
      else
        line_text = btk_text_iter_get_text (start, &next);
    }
 ...
 * buffers.
 * 
 * If the #BTK_TEXT_SEARCH_VISIBLE_ONLY flag is present, the match may
 * have invisible text interspersed in @str. i.e. @str will be a
 * possibly-noncontiguous subsequence of the matched range. similarly,
 * if you specify #BTK_TEXT_SEARCH_TEXT_ONLY, the match may have
                              ...
                              const BtkTextIter *limit)
{
  ...
  if (*str == '\0')
    {
      ...
      if (btk_text_iter_forward_char (&match))
        {
          if (limit &&
              btk_text_iter_equal (&match, limit))
            return FALSE;
  ...
  visible_only = (flags & BTK_TEXT_SEARCH_VISIBLE_ONLY) != 0;
  slice = (flags & BTK_TEXT_SEARCH_TEXT_ONLY) == 0;
  ...
  do
    {
       ...
       * btk_text_iter_get_text () is called repeatedly on
       * a single line.
       */
      BtkTextIter end;
      ...
      if (lines_match (&search, (const gchar**)lines,
                       visible_only, slice, &match, &end))
        {
          if (limit == NULL ||
              (limit &&
               btk_text_iter_compare (&end, limit) <= 0))
            {
              retval = TRUE;
                   ...
                   const BtkTextIter *start)
{
  ...
  btk_text_iter_set_line_offset (&line_start, 0);

  if (btk_text_iter_equal (&line_start, &line_end))
    {
      /* we were already at the start; so go back one line */
      btk_text_iter_backward_line (&line_start);
    }
  ...
  while (i >= 0)
    {
      ...
      if (win->slice)
        {
          if (win->visible_only)
            line_text = btk_text_iter_get_visible_slice (&line_start, &line_end);
          else
            line_text = btk_text_iter_get_slice (&line_start, &line_end);
        }
      else
        {
          if (win->visible_only)
            line_text = btk_text_iter_get_visible_text (&line_start, &line_end);
          else
            line_text = btk_text_iter_get_text (&line_start, &line_end);
        }
...
lines_window_back (LinesWindow *win)
{
  ...
  if (!btk_text_iter_backward_line (&new_start))
    return FALSE;
  else
    {
      ...
      btk_text_iter_forward_line (&win->first_line_end);
    }

  if (win->slice)
    {
      if (win->visible_only)
        line_text = btk_text_iter_get_visible_slice (&win->first_line_start,
                                                     &win->first_line_end);
      else
        line_text = btk_text_iter_get_slice (&win->first_line_start,
                                             &win->first_line_end);
    }
  else
    {
      if (win->visible_only)
        line_text = btk_text_iter_get_visible_text (&win->first_line_start,
                                                    &win->first_line_end);
      else
        line_text = btk_text_iter_get_text (&win->first_line_start,
                                            &win->first_line_end);
    }
...
/**
 * btk_text_iter_backward_search:
 * @iter: a #BtkTextIter where the search begins
 * @str: search string
 ...
 *
 * Same as btk_text_iter_forward_search(), but moves backward.
 *
 * Return value: whether a match was found
 **/
gboolean
                               ...
                               const BtkTextIter *limit)
{
  ...
  if (limit &&
      btk_text_iter_compare (limit, iter) > 0)
    return FALSE;
  
  if (*str == '\0')
    {
      /* If we can move one char, return the empty string there */
      BtkTextIter match = *iter;

      if (limit && btk_text_iter_equal (limit, &match))
        return FALSE;
      
      if (btk_text_iter_backward_char (&match))
        {
          if (match_start)
            *match_start = match;
          if (match_end)
  ...
  do
    {
      ...
      if (first_line_match &&
          vectors_equal_ignoring_trailing (lines + 1, win.lines + 1))
        {
          ...
          if (limit &&
              btk_text_iter_compare (limit, &start_tmp) > 0)
...
{
  BtkTextRealIter *real_lhs;
  BtkTextRealIter *real_rhs;

  real_lhs = (BtkTextRealIter*)lhs;
  real_rhs = (BtkTextRealIter*)rhs;
                       ...
                       const BtkTextIter *rhs)
{
  ...
  real_lhs = btk_text_iter_make_surreal (lhs);
  real_rhs = btk_text_iter_make_surreal (rhs);
                        ...
                        const BtkTextIter *end)
{
  ...
  g_return_val_if_fail (btk_text_iter_compare (start, end) <= 0, FALSE);
  
  return btk_text_iter_compare (iter, start) >= 0 &&
 ...
 * btk_text_iter_order:
 * @first: a #BtkTextIter
 * @second: another #BtkTextIter
 *
 * Swaps the value of @first and @second if @second comes before
 * @first in the buffer. That is, ensures that @first and @second are
 * in sequence. Most text buffer functions that take a range call this
 * automatically on your behalf, so there's no real reason to call it yourself
 * in those cases. There are some exceptions, such as btk_text_iter_in_range(),
 * that expect a pre-sorted range.
 * 
 **/
void
btk_text_iter_order (BtkTextIter *first,
                     BtkTextIter *second)
{
  g_return_if_fail (first != NULL);
  g_return_if_fail (second != NULL);

  if (btk_text_iter_compare (first, second) > 0)
    {
      BtkTextIter tmp;
                                  ...
                                  gint char_index)
{
  ...
  line = _btk_text_btree_get_line_at_char (tree, char_index,
                                       ...
                                       gint          char_on_line)
{
  ...
  line = _btk_text_btree_get_line_no_last (tree, line_number, &real_line);
                                          ...
                                          BtkTextTag     *tag)
{
  ...
  line = _btk_text_btree_first_could_contain_tag (tree, tag);

  if (line == NULL)
    {
      /* Set iter to last in tree */
      _btk_text_btree_get_end_iter (tree, iter);
      check_invariants (iter);
      return FALSE;
    }
  else
                                       ...
                                       const gchar *mark_name)
{
  ...
  mark = _btk_text_btree_get_mark_by_name (tree, mark_name);

  if (mark == NULL)
    return FALSE;
  else
    {
      _btk_text_btree_get_iter_at_mark (tree, iter, mark);
      check_invariants (iter);
      return TRUE;
    }
}
...
void
_btk_text_btree_get_iter_at_mark (BtkTextBTree *tree,
                                  BtkTextIter *iter,
                                  BtkTextMark *mark)
{
  BtkTextLineSegment *seg;
  ...
  g_assert (seg->body.mark.line == _btk_text_iter_get_text_line (iter));
  check_invariants (iter);
}

void
_btk_text_btree_get_iter_at_child_anchor (BtkTextBTree       *tree,
                                          BtkTextIter        *iter,
                                          BtkTextChildAnchor *anchor)
{
  ...
  g_assert (seg->body.child.line == _btk_text_iter_get_text_line (iter));
  check_invariants (iter);
}

void
_btk_text_btree_get_end_iter         (BtkTextBTree   *tree,
                                      BtkTextIter    *iter)
{
  g_return_if_fail (iter != NULL);
  g_return_if_fail (tree != NULL);

  _btk_text_btree_get_iter_at_char (tree,
                                   iter,
                                   _btk_text_btree_char_count (tree));
  check_invariants (iter);
}
...
_btk_text_iter_check (const BtkTextIter *iter)
{
  ...
  g_assert (sizeof (BtkTextIter) == sizeof (BtkTextRealIter));

  if (real->chars_changed_stamp !=
      _btk_text_btree_get_chars_changed_stamp (real->tree))
  ...
  segments_updated = (real->segments_changed_stamp ==
                      _btk_text_btree_get_segments_changed_stamp (real->tree));
  ...
  if (real->line_byte_offset >= 0)
    {
      _btk_text_line_byte_locate (real->line, real->line_byte_offset,
  ...
  if (real->line_char_offset >= 0)
    {
      _btk_text_line_char_locate (real->line, real->line_char_offset,
      ...
      if (char_segment->type == &btk_text_char_type)
        {
          ...
          if (!btk_text_byte_begins_utf8_char (char_segment->body.chars + seg_byte_offset))
  ...
  if (_btk_text_line_is_last (real->line, real->tree))
    g_error ("Iterator was on last line (past the end iterator)");
}

#define __BTK_TEXT_ITER_C__
